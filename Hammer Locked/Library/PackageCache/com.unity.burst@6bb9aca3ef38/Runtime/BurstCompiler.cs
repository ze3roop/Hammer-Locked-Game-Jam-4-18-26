...
namespace Unity.Burst
{
    ...
    public static class BurstCompiler
    {
        ...
        /// <param name="deterministicCompilation"></param>
        /// <returns></returns>
        /// <remarks>NOT AVAILABLE, unsafe to use</remarks>
        internal static unsafe T CompileDelegate<T>(T delegateMethod, bool deterministicCompilation = false) where T : class
        {
            // We have added support for runtime CompileDelegate in 2018.2+
            void* function = Compile(delegateMethod, false, deterministicCompilation);
        ...
        private static unsafe void* Compile(object delegateObj, bool isFunctionPointer, bool deterministicCompilation = false)
        {
            ...
            return Compile(delegateMethod, delegateMethod.Method, isFunctionPointer, false, deterministicCompilation);
        }

        private static unsafe void* Compile(object delegateObj, MethodInfo methodInfo, bool isFunctionPointer, bool isILPostProcessing, bool deterministicCompilation = false)
        {
            ...
            if (Options.TryGetOptions(methodInfo, out extraOptions, isForILPostProcessing: isILPostProcessing, deterministicCompilation: deterministicCompilation))
            {
        ...
        internal static void InitialiseDebuggerHooks()
        {
            if (IsApiAvailable("BurstManagedDebuggerPluginV1") &&
                string.IsNullOrEmpty(Environment.GetEnvironmentVariable("BURST_DISABLE_DEBUGGER_HOOKS")) &&
                UnityEditor.Scripting.ManagedDebugger.isEnabled)
            {
